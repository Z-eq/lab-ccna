...
══════════════════════════════════════════════════════════
CRITICAL: The topology field is parsed by an SVG renderer. Follow these rules EXACTLY:

MACHINE-PARSEABLE TOPOLOGY LINES (required, one connection per line):
Each connection must appear on its own line with BOTH device names and the word "Trunk" or "Link":
  "R1(E0/0) Trunk SW1(E0/0)"   ← trunk link, word "Trunk" MUST appear
  "SW1(E0/2) Trunk SW2(E0/0)"  ← trunk between switches
  "SW1(E0/1) Link PC1"         ← access link to PC
  "SW2(E0/1) Link PC2"         ← access link to PC

AFTER the machine-parseable lines, you may add human-readable ASCII art with boxes and descriptions.

DEVICE TYPES the renderer understands:
  "router" — shown as blue box, R badge
  "switch" — shown as green box, SW badge  
  "pc"     — shown as dashed grey box with 💻 icon

The renderer auto-places: routers at top, switches in middle, PCs at bottom.
ALWAYS include PC devices in the "devices" array when the lab mentions PCs.

ROUTER-TO-ROUTER EXAMPLE:
"topology": "R1(E0/0) Link R2(E0/0)\\n\\n         ┌──────────────────────┐         ┌──────────────────────┐\\n         │          R1          │         │  Lo0: 2.2.2.2/32     │\\n         │  Lo0: 1.1.1.1/32     │         └──────────────────────┘\\n         └──────────────────────┘\\n                    10.0.12.0/30"

ROUTER-ON-A-STICK EXAMPLE:
"topology": "R1(E0/0) Trunk SW1(E0/0)\\nSW1(E0/2) Trunk SW2(E0/0)\\nSW1(E0/1) Link PC1\\nSW2(E0/1) Link PC2\\n\\nR1 acts as inter-VLAN router via subinterfaces\\nVLAN 10: 10.0.10.0/24 (GW: 10.0.10.1)\\nVLAN 20: 10.0.20.0/24 (GW: 10.0.20.1)\\nNative VLAN 88 (untagged)"

SWITCHING EXAMPLE:
"topology": "SW1(E0/0) Trunk SW2(E0/0)\\nSW1(E0/1) Link PC1\\nSW2(E0/1) Link PC2\\n\\nSW1 ══ Trunk (VLAN 10,20,99) ══ SW2\\n│PC1 VLAN 10          PC2 VLAN 20│"
