...
function detectLinks(devices, topoText) {
  ...
  if (links.length===0 && devices.length>=2) {
    for (let i=0;i<devices.length-1;i++) {
      const getFirst = d => Object.keys(d.interfaces||{}).find(n=>!/loopback/i.test(n))||"E0/0";
      addLink(i,i+1,getFirst(devices[i]),getFirst(devices[i+1]),"","",false);
    }
  }

  // ── 4. Auto-promote to Trunk ───────────────────────────────────────────────
  // Any link between a router and a switch where neither has an IP match is likely a trunk
  links.forEach(lk => {
    if (lk.isTrunk) return;
    const tA = getTier(devices[lk.from]);
    const tB = getTier(devices[lk.to]);
    // Router ↔ Switch with no IP (subinterfaces case) → trunk
    if (!lk.fromIp && ((tA===0&&tB===1)||(tA===1&&tB===0))) {
      lk.isTrunk = true;
    }
  });
